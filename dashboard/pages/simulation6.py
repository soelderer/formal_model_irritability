            ...
            html.Div([
                dbc.Card([
                    ...
                    html.Div([
                        ...
                        id=f"{page_prefix + page_id}-I-slider_div"),
                    dbc.Tooltip(
                        shared_content.create_tooltip_text("I"),
                        target=f"{page_prefix + page_id}-I-slider_div",
                        placement="top",
                    ),
...
def update_graph(store):
    lambda_A, eta, gamma, alpha, kappa, lambda_C, midpoint_C, w_v_A, \
        I, iteration = (store[k] for k in [
            "lambda_A", "eta", "gamma", "alpha", "kappa", "lambda_C",
            "midpoint_C", "w_v_A", "I", "iteration"
        ])
    ...
    # Single model iteration
        ...
        table = pq.read_table(
            os.path.join(
                config.DATA_DIR,
                "006_response_inhibition",
                f"006_response_inhibition_{iteration}.parquet"
            ),
            columns=cols_needed,
            filters=filters
        )
